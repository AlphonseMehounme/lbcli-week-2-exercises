...
# WRITE YOUR SOLUTION BELOW:
FIRST_OUTPUT_VALUE=$(bitcoin-cli -regtest decoderawtransaction $BASE_TX | jq -r '.vout[0].value * 100000000 | round')
...
UTXO_VALUE=$(bitcoin-cli -regtest decoderawtransaction $BASE_TX | jq -r '.vout['$UTXO_VOUT_INDEX'].value * 100000000 | round')
...
CHANGE_AMOUNT=$(($UTXO_VALUE - $FEE_SATS - $PAYMENT_AMOUNT))
check_cmd "Change calculation" "CHANGE_AMOUNT" "$CHANGE_AMOUNT"

# Convert amounts to BTC for createrawtransaction
PAYMENT_BTC=$(echo "scale=8; $PAYMENT_AMOUNT/100000000" | bc | awk '{printf "%.8f\n", $1}')
echo $PAYMENT_BTC
CHANGE_BTC=$(echo "scale=8; $CHANGE_AMOUNT/100000000" | bc | awk '{printf "%.8f\n", $1}')
echo $CHANGE_BTC
...
# WRITE YOUR SOLUTION BELOW:
VERIFY_RBF=$(echo $DECODED_TX | jq 'if .vin[0].sequence < 4294967295 then true else false end')
...
# Convert to BTC
CHILD_SEND_BTC=$(echo "scale=8; $CHILD_SEND_AMOUNT/100000000" | bc | awk '{printf "%.8f\n", $1}')
...
# WRITE YOUR SOLUTION BELOW:
TIMELOCK_INPUTS='[{"txid":"'$SECONDARY_TXID'","vout":1}]'
...
SECONDARY_OUTPUT_VALUE=$(bitcoin-cli -regtest decoderawtransaction $SECONDARY_TX | jq -r '.vout[1].value * 100000000 | round')
...
# Convert to BTC
TIMELOCK_BTC=$(echo "scale=8; $TIMELOCK_AMOUNT/100000000" | bc | awk '{printf "%.8f\n", $1}')
...
TIMELOCK_TX=$(bitcoin-cli -regtest createrawtransaction $TIMELOCK_INPUTS $TIMELOCK_OUTPUTS 10)
