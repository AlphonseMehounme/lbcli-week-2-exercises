# Create a raw transaction with an amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
raw_tx=""
recipient="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"
utxo_txid=$(bitcoin-cli -regtest decoderawtransaction $raw_tx | jq -r '.txid')
#echo $trxid
#trx=$(bitcoin-cli decoderawtransaction $raw_tx)
#echo $trx
rawtxhex=$(bitcoin-cli -regtest createrawtransaction '''[{ "txid": "'$utxo_txid'", "vout": 0 }, { "txid": "'$utxo_txid'", "vout": 1}]''' '''{ "'$recipient'": 0.20000000 }''')
echo $rawtxhex
